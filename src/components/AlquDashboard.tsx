  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-xl print:fixed print:inset-0 print:max-h-none print:max-w-none print:translate-x-0 print:translate-y-0 print:border-0">
    ...
    <div className="space-y-4 rounded-lg border border-border bg-card p-5">
      ...
      <div className="border-y border-border py-3 text-sm">
        ...
        <div className="mt-1 flex justify-between"><span className="text-muted-foreground">Base + mínimo</span><span>{eur.format(baseLuz)}</span></div>
        <div className="mt-1 flex justify-between"><span className="text-muted-foreground">Luz con IVA ({dec.format(n(tenant.iva))} %)</span><strong>{eur.format(n(receipt.total_luz))}</strong></div>
      </div>
